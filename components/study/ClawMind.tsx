...
export function ClawMind() {
  ...
  async function send() {
    ...
    try {
      ...
      setMessages(m => [...m, { id: crypto.randomUUID(), from: 'clawmind', text: data.reply }])
    } catch {
      setMessages(m => [...m, { id: crypto.randomUUID(), from: 'clawmind', text: 'Error: could not reach ClawMind' }])
    }
    setText('')
  }
  ...
  return (
    <div className="bg-card rounded-xl shadow-sm border border-border p-6 mb-6">
      ...
      <div className="space-y-2 mb-3">
        {messages.map((m) => (
          <div key={m.id} className={`p-2 rounded ${m.from === 'you' ? 'bg-blue-100/60 dark:bg-blue-900/30' : 'bg-muted'}`}>
